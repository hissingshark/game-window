...
// may not be required at all as eglut.h may supply the entire API
#ifdef EGLUT_USE_FB
#include <eglut_fb.h>
#else
#include <eglut_x11.h>
...
FBWindow* FBWindow::currentWindow;

FBWindow::FBWindow(const std::string& title, int width, int height, GraphicsApi api) :
...
void FBWindow::setIcon(std::string const &iconPath) {
...
void FBWindow::pollEvents() {
    eglutPollEvents();
}

void FBWindow::setCursorDisabled(bool disabled) {
    cursorDisabled = disabled;
    eglutSetMousePointerLocked(disabled ? EGLUT_POINTER_LOCKED : EGLUT_POINTER_UNLOCKED);
}

void FBWindow::setFullscreen(bool fullscreen) {
...
void FBWindow::setSwapInterval(int interval) {
    eglutSwapInterval(interval);
}

void FBWindow::_eglutIdleFunc() {
...
void FBWindow::_eglutDisplayFunc() {
    if (currentWindow == nullptr)
        return;
    currentWindow->onDraw();
}

void FBWindow::_eglutReshapeFunc(int w, int h) {
...
void FBWindow::_eglutMouseFunc(int x, int y) {
    if (currentWindow == nullptr)
        return;
    currentWindow->onMousePosition(x, y);
}

void FBWindow::_eglutMouseRawFunc(double x, double y) {
    if (currentWindow == nullptr)
        return;
    currentWindow->onMouseRelativePosition(x, y);
}

void FBWindow::_eglutMouseButtonFunc(int x, int y, int btn, int action) {
    if (currentWindow == nullptr)
        return;
    if (btn == 4 || btn == 5) {
...
int FBWindow::obtainTouchPointer(int eglutId) {
...
void FBWindow::releaseTouchPointer(int ourId) {
    pointerIds[ourId] = -1;
}

void FBWindow::_eglutTouchStartFunc(int id, double x, double y) {
    if (currentWindow == nullptr)
        return;
...
void FBWindow::_eglutTouchUpdateFunc(int id, double x, double y) {
    if (currentWindow == nullptr)
        return;
    int ourId = currentWindow->obtainTouchPointer(id);
...
void FBWindow::_eglutTouchEndFunc(int id, double x, double y) {
    if (currentWindow == nullptr)
        return;
    int ourId = currentWindow->obtainTouchPointer(id);
...
void FBWindow::_eglutKeyboardFunc(char str[5], int action) {
    if (currentWindow == nullptr ||
...
KeyCode FBWindow::getKeyMinecraft(int keyCode) {
...
void FBWindow::_eglutKeyboardSpecialFunc(int key, int action) {
    if (currentWindow == nullptr)
        return;
    if (key == 65507)
...
void FBWindow::_eglutPasteFunc(const char* str, int len) {
    if (currentWindow == nullptr)
        return;
    currentWindow->onPaste(std::string(str, len));
}

void FBWindow::_eglutFocusFunc(int action) {
    if (currentWindow == nullptr)
        return;
    LinuxGamepadJoystickManager::instance.onWindowFocused(currentWindow, (action == EGLUT_FOCUSED));
}

void FBWindow::_eglutCloseWindowFunc() {
    if (currentWindow == nullptr)
        return;
    currentWindow->onClose();
}

void FBWindow::getWindowSize(int& width, int& height) const {
    width = this->width;
    height = this->height;
}

void FBWindow::setClipboardText(std::string const &text) {
